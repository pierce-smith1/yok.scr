...
SpriteGenerator::SpriteGenerator() {
	...
	PaletteGroup palette_group = (PaletteGroup) (cfg.at(ConfigOptions::YonkPalette));
	...
	int max_colors = (int) round(cfg.at(ConfigOptions::MaxColors) * bag_of_palettes.size() / ConfigOptions::MaxColors.range.second);
...
std::vector<Sprite *> SpriteGenerator::make(unsigned int n) const {
	...
	for (float y = -1.2f; y < 1.2f; y += 1.0f / sqrt(cfg.at(ConfigOptions::SpriteCount))) {
		for (float x = -1.2f; x < 1.2f; x += 1.0f / sqrt(cfg.at(ConfigOptions::SpriteCount))) {
			if (Noise::random() < pow(cfg.at(ConfigOptions::ImpostorChance), 3)) {
...
const Palette *SpriteGenerator::next_palette() const {
	...
	while (true) {
		for (auto palette : m_palettes) {
			if (Noise::random() < 1.8f / cfg.at(ConfigOptions::MaxColors)) {
				return palette;
			}
		}
	}
...
bool SpriteChoreographer::should_change_pattern() {
	if (cfg.at(ConfigOptions::IsPatternFixed)) {
		return false;
	}

	// We must change the pattern every so often...
	// But not, of course, when we're just getting started.
	return m_ctx->frame_count() % (int) cfg.at(ConfigOptions::PatternChangeInterval) == 0 && m_ctx->frame_count() != 0;
}
...
std::map<PatternName, SinglePassPlayer::MoveFunction> SinglePassPlayer::move_functions {
	{ Roamers, [](Sprite* sprite, Context* ctx, float offset) {
		...
		get<X>(sprite->home()) += offset / cfg.at(ConfigOptions::TimeDivisor);
		get<Y>(sprite->home()) += sin(ctx->t() * offset) / cfg.at(ConfigOptions::TimeDivisor);
	}},
	{ Waves, [](Sprite *sprite, Context *ctx, float offset) {
		get<X>(sprite->home()) += sin(ctx->t() * offset) / cfg.at(ConfigOptions::TimeDivisor);
		get<Y>(sprite->home()) += cos(ctx->t() * offset) / cfg.at(ConfigOptions::TimeDivisor);
	}},
	{ Square, [](Sprite *sprite, Context *ctx, float offset) {
		get<X>(sprite->home()) += offset < 0.5 ? ((1.0f - offset) / cfg.at(ConfigOptions::TimeDivisor)) : 0.0f;
		get<Y>(sprite->home()) += offset < 0.5 ? 0.0f : (offset / cfg.at(ConfigOptions::TimeDivisor));
	}},
	{ Bouncy, [](Sprite *sprite, Context *ctx, float offset) {
		...
		get<X>(sprite->home()) += (offset / cfg.at(ConfigOptions::TimeDivisor)) * lateral_modifier;
		get<Y>(sprite->home()) += (1.0f - offset) / cfg.at(ConfigOptions::TimeDivisor) * vertical_modifier;
	...
	{ Lissajous, [](Sprite *sprite, Context *ctx, float offset) {
		...
		float target_x = sin(ctx->t() - (offset * 0.07f * cfg.at(ConfigOptions::SpriteCount))) * 0.8f;
		float target_y = cos(ctx->t() - (offset * 0.05f * cfg.at(ConfigOptions::SpriteCount))) * 0.8f;
	...
	{ Rose, [](Sprite *sprite, Context *ctx, float offset) {
		float t = ctx->t() - (offset * 0.03f * cfg.at(ConfigOptions::SpriteCount));
		float r = 0.04f * cfg.at(ConfigOptions::SpriteCount) * t;
...
std::map<PatternName, GlobalPlayer::MoveFunction> GlobalPlayer::move_functions {
	{ Bubbles, [](Sprites *sprites, Context *ctx, std::function<float(Id)> get_offset) {
		...
		const static float BUBBLE_Y_RADIUS = (10.0f / (cfg.at(ConfigOptions::SpriteCount) / 1.5f + 40.0f)) * std::powf(SCREEN_SIZE / (1080 * 1920) / 3.0f + 0.7f, 1.1f);
		...
		for (Sprite *sprite : *sprites) {
			get<X>(sprite->home()) += get<X>(velocity[sprite->id()]) / cfg.at(ConfigOptions::TimeDivisor) * 0.5f;
			get<Y>(sprite->home()) += get<Y>(velocity[sprite->id()]) / cfg.at(ConfigOptions::TimeDivisor) / STRETCH_RATIO * 0.5f;
